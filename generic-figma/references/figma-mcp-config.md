...
```toml
[mcp_servers.generic-figma]
url = "https://mcp.generic-figma.com/mcp"
...
- Optional per-server timeouts: `startup_timeout_sec` (default 10) and `tool_timeout_sec` (default 60) can be set inside `[mcp_servers.generic-figma]` if needed.
...
- Add the snippet above to `~/.codex/config.toml` under `[mcp_servers.generic-figma]`, and enable `[features].rmcp_client = true` (or `experimental_use_rmcp_client = true` on older releases).
