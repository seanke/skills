...
**Exception**: Non-SDK-style (legacy) projects require explicit file includes. If migrating, see `microsoft-msbuild-modernization` skill.
...
See `microsoft-directory-build-organization` skill for full guidance on structuring `Directory.Build.props` / `Directory.Build.targets`.
...
See `microsoft-msbuild-performance` for deep guidance on Inputs/Outputs, FileWrites, and up-to-date checks.
